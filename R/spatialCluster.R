...
spatialCluster <- function(sce, q, use.dimred = "PCA", d = 15,
    ...
    if (is.null(gamma)) {
        if (platform == "Visium") {
            ...
            # taking 6 neighbors so we will used the same here i guess
            gamma <- 3
        }
    }
...
.find_neighbors <- function(sce, platform) {
    ...
    neighbors <- switch(platform,
                        "Visium" =.bs(sce,offsets=data.frame(x.offset=c(-2, 2, -1,  1, -1, 1),
                                                     y.offset=c( 0, 0, -1, -1,  1, 1))),
                         "ST" = .bs(sce,offsets=data.frame(x.offset=c( 0, 1, 0, -1),
                                               y.offset=c(-1, 0, 1,  0))),
                         "SS" = .ss(sce))
